...
void GraphWindow::addGraph(GRAPHS graph, ALG_VARIANT _alg, Qt::GlobalColor color, Qt::PenStyle linestyle, string vAxis, string name, QCPAxis *keyAxis, QCPAxis *valueAxis)
{
    if(checkIfGraphExist(graph))
    {
        graph_components.find(graph)->second->addGraph(_alg, color, linestyle, vAxis, keyAxis, valueAxis, name);
        return;
    }
